...
try:
    from abstract.pyAASScheduler import Scheduler
except ImportError:
    from  main.abstract.pyAASScheduler import Scheduler
...
class testsample(object):
    ...
    def configureLogger(self):
        ...
        self.fileLogger_Handler = logging.FileHandler(self.base_dir+"/logs/testsample.LOG")
    ...
    def configureSkills(self): 
        ...
        for aasDetail in self.aasSkills.keys():
            self.skillDetails = self.aasSkills[aasDetail]
            skillList = []
            for skill in self.skillDetails.keys():
                ...
                self.skillInstanceDict[skill] = {"skillInstance":skillInstance,"skillDetails":self.skillDetails[skill]}
                skillList.append(skill)
            self.skillListWeb[aasDetail] = skillList
    ...
    def configueThingDescriptionProperties(self):
        tdSubmodel = self.aasConfigurer.getRelevantSubModel("ThingDescription")
        self.tdProperties = self.aasConfigurer.configureThingDescriptionProperties(tdSubmodel)
    ...
    def startSkills(self):      
        ...
        for skill in self.skillInstanceDict.keys():
            skillInfo = self.skillInstanceDict[skill] 
            skillInstance = skillInfo["skillInstance"]
            skillDetails = skillInfo["skillDetails"]
            skillInstanceTh = threading.Thread(target=skillInstance.Start, args=(self.msgHandler, skillDetails,))
