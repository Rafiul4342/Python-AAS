...
class excProductionStepSeq(object):
    ...
    def excProductionStepSeq_Logic(self):
        skillsList = list(self.baseClass.pyAAS.skillDetailsDict.keys())
        if len(self.baseClass.pyAAS.productionSequenceList) == 0:
            ...
            for productionStep in self.baseClass.pyAAS.productionSequenceList:
                if productionStep not in skillsList:
                    self.sendProductionStepOrder_Enabled = False
                    self.sendCompletionResponse_Enabled = False
                    self.baseClass.responseMessage["status"] = "E"
                    self.baseClass.responseMessage["code"] = "E00.12"
                    self.baseClass.responseMessage["message"] = "The execution sequence is not correct, please alter production step submodel." 
                else:
                    self.sendCompletionResponse_Enabled = False
                    self.sendFailureResponse_Enabled = False
...
class sendFailureResponse(object):
    ...
    def sendFailureResponse_Logic(self):
        self.InElem = self.baseClass.pyAAS.dba.getAAsSubmodelsbyId(self.baseClass.pyAAS.AASID,"StatusResponse")["message"]
...
class sendProductionStepOrder(object):
    ...
    def sendProductionStepOrder_Logic(self):
        self.productionStep = self.baseClass.pyAAS.productionSequenceList[0]
        del self.baseClass.pyAAS.productionSequenceList[0]
    ...
    def create_Outbound_Message(self):
        ...
        for oMessage in self.oMessages:
            ...
            # For the return reply these details could be obtained from the inbound Message
            receiverId = self.baseClass.pyAAS.AASID#message["frame"]["sender"]["identification"]["id"]
    ...
    def run(self):
        ...
        if (InputDocument != "NA"):
            ...
            while (((self.baseClass.waitforStepOrderCompletion_Queue).qsize()) == 0):
                print((self.baseClass.waitforStepOrderCompletion_Queue).qsize())
                time.sleep(1)
                i = i + 1 
...
class ProductionManager(object):
    ...
    def Start(self, msgHandler, skillDetails):
        ...
        self.listHandler = serviceLogHandler(self.msgHandler.ProductionManagerLogList)
